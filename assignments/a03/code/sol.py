...
def classify(X, theta, threshold=0.5):
   ...
   # Calculate the probabilities
    ...
    # Convert Boolean (True/False) to int (1/0)
...
def gradient(X, y, theta):
    ...
    # Gradient formula (1/n) * X_transpose * (predictions - actuals)
    return (X.T @ (h - y)) / n 
...
def gradient_descent(
    ...
    # We initialize theta_1 just in case the loop doesn't run
    theta_1 = theta_0
    ...
    for i in range(max_iter):
        ...
        # 1. Calculate gradient using current theta (theta_0)
        grad = gradient(X, y, theta_0)

        # 2. Use gradient to find theta_1 
