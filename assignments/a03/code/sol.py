...
def sigma(X, theta):
    ...
    ----------
    X: numpy.ndarray
        Feature matrix of shape (n, p) containing the observed characteristics
        of each observation. Its first column is always full of ones.
    theta: numpy.ndarray
        The parameters of a logistic regression model. Its shape is (p,).

    Returns
    -------
    Numpy ndarray of shape (n,) representing the predicted probabilities for
    each observation.
    """
    z = X @ theta
    z = np.clip(z, -500, 500)
...
def classify(X, theta, threshold=0.5):
    """Classifies all observations in the data based on their predicted
    probabilities.

    This function calls `sigma(X, theta)` to calculate the array of predicted
    probabilities and then classifies each observation using `threshold`. }
    If a given probability is greater than `threshold`,
    it labels it as `1` and `0` otherwise.
    ...
    theta: numpy.ndarray
        The parameters of a logistic regression model. Its shape is (p,).
    threshold: float
        A float between 0.0 and 1.0. Observations whose predicted probabilities
        are strictly greater than this threshold are classified
        as 1 and 0 otherwise. Defaults to 0.5.

    Returns
    -------
    Numpy ndarray of shape (n,) representing the predicted labels
    for each observation.
    """
    prob = sigma(X, theta)  # (n,)
...
def loss(X, y, theta):
    ...
    epsilon = 1e-15
    cost = -(1.0 / n) * np.sum(
        y * np.log(h + epsilon) + (1.0 - y) * np.log(1.0 - h + epsilon)
    )
    return float(cost)
...
def gradient(X, y, theta):
    ...
    ----------
    X: numpy.ndarray
        Feature matrix of shape (n, p) containing the observed
    ...
    Returns
    -------
    Numpy ndarray of shape (p,) representing the gradient of the loss
    function at theta.
    """
...
def gradient_descent(
    ...
    This function is an implementation of the original gradient descent
    algorithm for logistic regression.
    It minimizes the negative loss-likelihood cross entropy loss
    ...
    min_gain: float
        The minimum gain in the loss function to trigger another iteration.
        If the updated parameters, theta, fail to cause a gain larger than
        this threshold, the algorithm comes to an early stop.
        Defaults to 0.00001.
    step_size: Float
        This factor represents the fraction of the gradient used to update
        theta at each iteration (`theta_1 = theta_0 - step_size * nabla`).
        Defaults to 0.001.

    Returns
    -------
    Numpy ndarray of shape (p,) representing the final parameters
    estimated by the naive gradient descent algorithm.
    """
    X = np.asarray(X, dtype=float)
    y = np.asarray(y, dtype=float).ravel()
